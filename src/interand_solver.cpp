...
void gaussian_quadrature_integrate(ModelParams& params, CalcBuffer& buffer, SimResults& results, bool calc_grad){
    ...
    // 初始化梯度相关变量 (即使不计算梯度，保持定义以免编译错误，但不会被更新)
    ...
    long eva = static_cast<long>(2.0 * (3.0 + std::floor(3.0 / ((params.evaluation(0) / params.a) + 1.0))));
    ...
    for (int i = 0; i < 119; ++i){
        int calculated_n = static_cast<int>(std::ceil(n_points_number + 2.0 / std::sqrt(i + 1.0))) * 4;
        ...
        for (int j = 0; j < n_points; ++j){
            ...
            if (e_Index > 0.05){
                ...
                // 【优化】繁重的梯度求解部分
                if (calc_grad) {
                    Derivative_E(buffer, params); // 计算偏导数矩阵，开销大
                    for (int k = 1; k < 6; ++k){
                        compute_dKdE_times_X(buffer, k); // 矩阵乘法
                        Solve_dXdE(buffer);             // 线性方程组求解
        ...
        // 检查收敛 (基于位移值，所以不受 calc_grad 影响)
        if (std::abs(interval_integral) < 0.001 * std::abs(total_integral) && (i % 2 == 1) && (i > eva)) {
            ...
            if (calc_grad) {
                ...
                results.gradient_E(4) = final_gradient(4) * factor_common_2;
                 ...
                 // 如果不计算梯度，建议将其显式设为0，或者保持 InputMat 初始状态
